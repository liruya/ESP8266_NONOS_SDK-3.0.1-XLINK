...
LOCAL void ESPFUNC user_apconfig_send_info() {
    ...
    if (wifi_get_macaddr(STATION_IF, mac)) {
        ...
        if (root == NULL) {
            app_loge("create json object failed.");
            return;
        }
...
LOCAL void ESPFUNC user_apconfig_recv_cb(void *arg, char *pdata, uint16_t len) {
    ...
    app_logd("%d.%d.%d.%d:%d", premote->remote_ip[0], premote->remote_ip[1], premote->remote_ip[2], premote->remote_ip[3], premote->remote_port);
    ...
    if (pdes == NULL) {
        app_loge("apconfig error: os_zalloc failed");
        return;
    }
    ...
    if (root == NULL) {
        os_free(pdes);
        app_loge("cJSON parse receive failed...");
        return;
    }
    ...
    if (zone > 1200 || zone < -1200 || zone%100 >= 60 || zone%100 <= -60) {
        os_free(pdes);
        cJSON_Delete(root);
        app_loge("ap info invalid zone.");
        return;
    }
    apc_task->apc_result.zone = zone;
    app_logd("zone: %d", apc_task->apc_result.zone);
...
LOCAL bool ESPFUNC user_apconfig_start(task_t **task) {
    app_logd("apconfig start... %d  *%d  **%d", task, *task, **task);
    ...
    app_logd("espconn accept error: %d", error);
    ...
    app_logd("espconn create error: %d", error);
#endif
...
LOCAL void ESPFUNC user_apconfig_disconn_cb(void *arg) {
    app_logd("espconn diconnect callback");
}
#endif
...
LOCAL void ESPFUNC user_apconfig_stop_cb(void *arg) {
    ...
    int8_t error = 0;
        ...
        app_logd("espconn tcp disconnect error: %d", error);
        error = espconn_delete(&ptask->server);
        app_logd("espconn tcp delete error: %d", error);
        os_free(ptask->server.proto.tcp);
    #else
        error = espconn_delete(&ptask->server);
        app_logd("espconn udp delete error: %d", error);
...
LOCAL bool ESPFUNC user_apconfig_stop(task_t **task) {
    app_logd("apconfig stop... %d  *%d  **%d", task, *task, **task);
    if (task == NULL || *task == NULL) {
        return false;
    }
    apconfig_task_t *ptask = (apconfig_task_t *) (*task);
...
void ESPFUNC user_apconfig_instance_start(const task_impl_t *impl, const uint32_t timeout, const char *ssid, void (* const done)(int16_t)) {
    if (apc_task != NULL) {
        app_loge("apconfig start failed -> already started...");
        return;
    }
    apc_task = os_zalloc(sizeof(apconfig_task_t));
    if (apc_task == NULL) {
        app_loge("apconfig start failed -> malloc apc_task failed...");
        return;
    }
    app_logd("apconfig create... %d", apc_task);
